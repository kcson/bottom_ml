...
def load_local_images():
    ...
    dataset_count = 1
    while dataset_count <= 1000:
        for sub_dir in root_dir:
            if sub_dir.startswith('.'):
                continue
            sub_dir_full = os.path.join(root, sub_dir)
            image_file = os.path.join(sub_dir_full, '{}.jpeg'.format(dataset_count))
            label = labels.index(sub_dir)
            src = cv.imread(image_file, cv.IMREAD_GRAYSCALE)
            ...
            src = src.reshape(-1, 28, 28)
            if dataset_count <= 700:
                x_train.append(src)
